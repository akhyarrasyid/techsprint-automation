...
function KPIContent() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ['kpi', scenario],
    queryFn: () => fetchKPI(scenario),
  });
  ...
  if (!data) {
    return <EmptyState title="Belum ada data KPI" description="Upload file sales history dan jalankan pipeline terlebih dahulu." />;
  }
...
export default function KPIPage() {
  return (
    <Suspense fallback={<div className="grid grid-cols-4 gap-4">{Array(8).fill(0).map((_, i) => <div key={i} className="h-32 bg-white rounded-2xl animate-pulse" />)}</div>}>
      <KPIContent />
    </Suspense>
  );
}
